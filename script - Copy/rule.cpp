...
namespace cc::script
{
    ...
    bool validate(rules const& rules)
    {
        for (cc::pair<string, rule> const& r : rules)
        {
            ...
            // verify that each referenced constraint is found
            for (rule::block const& b : r.second.blocks)
            {
                for (constraint const& cons : b.set)
                {
                    if (cons.type() == constraint_type::kAsConstraint)
                    {
                        bool const found = rules.end() != rules.find(cons.asString());
                        if (!found)
                        {
                            assert(found);
                            return false;
